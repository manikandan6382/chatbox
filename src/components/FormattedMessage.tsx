...
export const FormattedMessage: React.FC<FormattedMessageProps> = ({ text, isStreaming = false }) => {
    ...
    return parts.map((part, idx) => {
      ...
      if (part.startsWith('`') && part.endsWith('`')) {
        return (
          <code 
            key={idx} 
            className="px-1.5 py-0.5 rounded-md bg-slate-100 dark:bg-white/10 text-amber-600 dark:text-amber-300 font-mono text-xs"
          >
            {part.slice(1, -1)}
          </code>
        );
  ...
  lines.forEach((line, idx) => {
    ...
    if (trimmed.startsWith('### ') || trimmed.startsWith('## ') || trimmed.startsWith('# ')) {
      ...
      elements.push(
        <h4 key={idx} className="text-[15px] font-bold text-slate-900 dark:text-white mt-3 mb-1 tracking-tight">
    ...
    if (/^(\*|-|•)\s+/.test(trimmed)) {
      ...
      elements.push(
        <div key={idx} className="flex items-start gap-2.5 my-1 pl-1">
          ...
          <div className="flex-1 text-[14px] leading-relaxed text-slate-800 dark:text-slate-200">
    ...
    if (/^\d+\.\s+/.test(trimmed)) {
      ...
      elements.push(
        <div key={idx} className="flex items-start gap-2.5 my-1.5 pl-1">
          <span className="w-5 h-5 rounded-full bg-slate-100 dark:bg-white/10 text-slate-700 dark:text-slate-300 font-mono text-[11px] font-bold flex items-center justify-center shrink-0 mt-0.5">
            {num}
          </span>
          <div className="flex-1 text-[14px] leading-relaxed text-slate-800 dark:text-slate-200">
    ...
    // Regular paragraph
    elements.push(
      <p key={idx} className="text-[14px] leading-relaxed text-slate-800 dark:text-slate-200 my-0.5">
        {renderInline(line)}
        {isLastLine && isStreaming && (
          <span className="inline-block w-2 h-4 ml-1 bg-amber-400 dark:bg-amber-300 rounded-xs animate-pulse align-middle shadow-[0_0_10px_rgba(245,158,11,0.6)]" />
        )}
